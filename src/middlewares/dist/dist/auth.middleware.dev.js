...
exports.authorizeVendorAccess = function (req, res, next) {
  ...
  if (!authReq.userRole || authReq.userRole !== "store_shopper" && authReq.userRole !== "vendor") {
    return res.status(403).json({
      message: 'Forbidden: Requires vendor staff or admin role.'
    });
  ...
  if (authReq.userRole === "store_shopper" && !authReq.vendorId) {
    return res.status(403).json({
      message: 'Forbidden: User is not associated with a vendor.'
    });
  }
...
exports.authorize = function (allowedRoles) {
  return function (req, res, next) {
    if (!req.userRole) {
      return res.status(403).json({
        error: 'Forbidden: User role not available on request.'
      });
    }
