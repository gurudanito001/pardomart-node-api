...
var router = express_1.Router(); // All routes below require a logged-in user

router.use(auth_middleware_1.authenticate); // --- Vendor-facing routes ---
...
router.patch('/:orderId/start-shopping', auth_middleware_1.authorizeVendorAccess, validation_middleware_1.validate(validation_middleware_1.validateVendorOrderAction), orderController.startShoppingController); // GET /api/v1/orders/vendor - Get all orders for the authenticated vendor's stores

router.get('/vendor', auth_middleware_1.authenticate, auth_middleware_1.authorize(['vendor']), orderController.getOrdersForVendor); // --- Customer-facing routes ---
...
router.patch('/:orderId/tip', validation_middleware_1.validate(validation_middleware_1.validateUpdateTip), orderController.updateOrderTipController); // --- Messaging within an Order ---
...
router.patch('/:orderId/messages/read', validation_middleware_1.validate(validation_middleware_1.validateMarkMessagesAsRead), messageController.markMessagesAsReadController); // --- Live Shopping Actions ---

router.patch('/:orderId/items/:itemId/update-shopping-status', auth_middleware_1.authorize([client_1.Role.vendor, client_1.Role.store_shopper, client_1.Role.shopper, client_1.Role.delivery_person]), validation_middleware_1.validate(validation_middleware_1.validateUpdateOrderItemShoppingStatus), orderController.updateOrderItemShoppingStatusController);
router.patch('/:orderId/items/:itemId/respond-to-replacement', auth_middleware_1.authorize([client_1.Role.customer]), validation_middleware_1.validate(validation_middleware_1.validateRespondToReplacement), orderController.respondToReplacementController); // Add a location point for a delivery person

router.post('/:orderId/delivery-location', auth_middleware_1.authenticate, auth_middleware_1.authorize([client_1.Role.delivery_person]), // Ensure this role exists
validation_middleware_1.validate(validation_middleware_2.validateAddLocation), deliveryPersonLocation_controller_1.addLocationController); // Get the delivery path for an order
